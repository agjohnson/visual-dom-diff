(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "jsdom", "./domIterator"], factory);
    }
})(function (require, exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    const jsdom_1 = require("jsdom");
    const domIterator_1 = require("./domIterator");
    ...
    test('iterate one text node', () => {
        expect(new domIterator_1.DomIterator(text).toArray()).toStrictEqual([text]);
    });
    test('iterate many nodes', () => {
        expect(new domIterator_1.DomIterator(fragment).toArray()).toStrictEqual([
            fragment,
            table,
            tr1,
            td1a,
    ...
    test('skip some nodes', () => {
        expect(new domIterator_1.DomIterator(fragment, {
    ...
    test('skip children of some nodes', () => {
        ...
        }).toArray()).toStrictEqual([fragment, table, tr1, tr2, img1, img2, img3]);
    });
    test('skip some nodes and their children', () => {
        expect(new domIterator_1.DomIterator(fragment, {
            skipChildren: node => node.nodeName === 'TR',
            skipSelf: node => node.nodeName === 'TR',
        }).toArray()).toStrictEqual([fragment, table, img1, img2, img3]);
    });
    ...
    test('skip the children of the root node', () => {
        ...
        }).toArray()).toStrictEqual([fragment]);
    });
    ...
    test('stay within root', () => {
        expect(new domIterator_1.DomIterator(tr1).toArray()).toStrictEqual([
            tr1,
            td1a,
            text1a,
            td1b,
    ...
    describe('forEach', () => {
        test('all', () => {
            const fn = jest.fn();
            const iterator = new domIterator_1.DomIterator(tr1);
            iterator.forEach(fn);
