...
import { areArraysEqual, areNodesEqual, charForNodeName, cleanUpNodeMarkers, diffText, getAncestors, isComment, isDocument, isDocumentFragment, isElement, isText, never, } from './util';
...
describe('areArraysEqual', () => {
    describe('default comparator', () => {
        ...
        test('identical nodes', () => {
            expect(areArraysEqual([text, text], [text, identicalText])).toBe(false);
        });
        test('different nodes', () => {
            expect(areArraysEqual([text, text], [text, differentText])).toBe(false);
        });
    });
    describe('node comparator', () => {
        test('the same nodes', () => {
            expect(areArraysEqual([text, text], [text, text], areNodesEqual)).toBe(true);
        });
        test('identical nodes', () => {
            expect(areArraysEqual([text, text], [text, identicalText], areNodesEqual)).toBe(true);
        });
        test('different nodes', () => {
            expect(areArraysEqual([text, text], [text, differentText], areNodesEqual)).toBe(false);
        });
    });
});
describe.each([
    ...
    describe('shallow', () => {
        ...
        test('elements with different attribute values', () => {
            expect(areNodesEqual(span, differentAttributeValuesSpan)).toBe(false);
        });
    ...
    describe('deep', () => {
        ...
        test('identical nodes', () => {
            expect(areNodesEqual(rootNode.cloneNode(true), rootNode.cloneNode(true), true)).toBe(true);
        });
        test('extraneous child', () => {
            ...
            expect(areNodesEqual(rootNode.cloneNode(true), differentRootNode, true)).toBe(false);
        });
...
describe('never', () => {
    test('default message', () => {
        expect(() => never()).toThrowError('visual-dom-diff: Should never happen');
    });
    test('custom message', () => {
...
describe('diffText', () => {
    ...
    test('word added with a node marker', () => {
        expect(diffText(`${pChar}start${pChar}end`, `${pChar}start${pChar}add${pChar}end`)).toStrictEqual([
    ...
    test('word removed with a node marker', () => {
        expect(diffText(`${pChar}start${pChar}remove${pChar}end`, `${pChar}start${pChar}end`)).toStrictEqual([
            [DIFF_EQUAL, `${pChar}start`],
    ...
    test('word replaced in text with node markers', () => {
        expect(diffText(`${pChar}start${pChar}remove${pChar}end`, `${pChar}start${pChar}add${pChar}end`)).toStrictEqual([
    ...
    describe('skip node markers when running diff_cleanupSemantic', () => {
        test('equal node markers only', () => {
            expect(diffText('\uE000\uE001\uE002\uE003', '\uE000\uE001\uE002\uE003')).toStrictEqual([[DIFF_EQUAL, '\uE000\uE001\uE002\uE003']]);
        });
        test('equal node markers with prefix', () => {
            expect(diffText('a\uE000\uE001\uE002\uE003', 'a\uE000\uE001\uE002\uE003')).toStrictEqual([[DIFF_EQUAL, 'a\uE000\uE001\uE002\uE003']]);
        });
        test('equal node markers with suffix', () => {
            expect(diffText('\uE000\uE001\uE002\uE003z', '\uE000\uE001\uE002\uE003z')).toStrictEqual([[DIFF_EQUAL, '\uE000\uE001\uE002\uE003z']]);
        });
        test('equal node markers with prefix and suffix', () => {
            expect(diffText('a\uE000\uE001\uE002\uE003z', 'a\uE000\uE001\uE002\uE003z')).toStrictEqual([[DIFF_EQUAL, 'a\uE000\uE001\uE002\uE003z']]);
        });
        ...
        test('changed node within text', () => {
            expect(diffText('prefix\uE000suffix', 'prefix\uE001suffix')).toStrictEqual([
                [DIFF_EQUAL, 'prefix'],
                [DIFF_DELETE, '\uE000'],
                [DIFF_INSERT, '\uE001'],
                [DIFF_EQUAL, 'suffix'],
        ...
        test.each([
            ...
            expect(diffText(`abcdef${string}ghijkl`, `123456${string}7890-=`)).toStrictEqual([
