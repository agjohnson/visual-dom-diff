(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "diff-match-patch", "./config", "./domIterator", "./util"], factory);
    }
    ...
    const getLength = (node) => (util_1.isText(node) ? node.length : 1);
    ...
    function visualDomDiff(oldRootNode, newRootNode, options = {}) {
        ...
        const config = config_1.optionsToConfig(options);
        const { addedClass, diffText, modifiedClass, removedClass, skipSelf, skipChildren, } = config;
        const notSkipSelf = (node) => !skipSelf(node);
        const getDepth = (node, rootNode) => util_1.getAncestors(node, rootNode).filter(notSkipSelf).length;
        const isFormattingNode = (node) => util_1.isElement(node) && skipSelf(node);
        const getFormattingAncestors = (node, rootNode) => util_1.getAncestors(node, rootNode)
